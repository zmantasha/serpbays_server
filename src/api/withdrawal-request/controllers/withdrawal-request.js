  ...
  async create(ctx) {
    try {
      ...
      try {
        ...
        if (userEmail) {
          console.log(`[WithdrawalRequest] Sending withdrawal request email to ${userEmail}`);

          // Create a transaction-like object for the email
          const transactionData = {
            ...
            notes: `Your withdrawal request for $${requestAmount} via ${method} has been received and is pending review.`
          };

          await emailService.sendWithdrawalRequestEmail(transactionData, userEmail, withdrawalRequest);
          console.log(`Withdrawal request email sent for withdrawal #${withdrawalRequest.id}`);
        }
      } catch (emailError) {
