...
module.exports = createCoreController('api::marketplace.marketplace', ({ strapi }) => ({
  ...
  async getCountryAnalytics(ctx) {
    try {
      // Import the country utility
      const { getCountryInfo } = require('../../../utils/countryFlags');
      ...
      entries.forEach(entry => {
        if (entry.countries && Array.isArray(entry.countries)) {
          ...
          const cleanCountry = entry.countries.trim().toLowerCase();
          if (cleanCountry) {
            countryStats[cleanCountry] = (countryStats[cleanCountry] || 0) + 1;
          }
        }
      ...
      // Convert to array format with flag data and sort by website count
